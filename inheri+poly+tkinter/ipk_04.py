...
def register_courses():
    ...
    if stu.classes:
        ...
        result.set(f"선택된 과목이 없습니다.")

def reset_all():
    var_py.set(0); var_ai.set(0); var_ds.set(0)
    stu.clearCourses()
    result.set("모든 선택을 해체했습니다.")

btu_frame = tk.Frame(root)
btu_frame.pack(pady=6)

tk.Button(btu_frame, text="등록하기", command=register_courses).pack(side="left", padx=8)
tk.Button(btu_frame, text="초기화",   command=reset_all).pack(side="left", padx=8)
